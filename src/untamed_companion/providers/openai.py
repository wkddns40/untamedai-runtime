...
class OpenAIProvider:
    ...
    async def generate_chat_response(self, state: ChatState) -> str:
        prompt = state.get("system_prompt") or "You are a conversational AI companion."
    ...
    async def generate_greeting(self, state: ChatState) -> str:
        ...
        prompt = state.get("system_prompt") or "Write one short first greeting."
        response = await self._client.chat.completions.create(
            model=self.chat_model,
            messages=[
                {"role": "system", "content": prompt},
    ...
    async def analyze_emotion(self, state: EmotionState) -> dict[str, object]:
        ...
        prompt = state.get("system_prompt") or (
            "Return JSON with primary_emotion, color_hex, summary_text, key_quote."
        )
        response = await self._client.chat.completions.create(
            model=self.chat_model,
            messages=[
